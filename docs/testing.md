...
# Confirm LAMMPS is in PATH on a compute node (not login node)
srun --partition=compute --pty bash -c "which lmp"
```
...
print(df[['comp_id','Tm','Ef','Sf','C0']].to_string())
...
```
This is acceptable for a connectivity test. Ef will be correct on compute nodes.

---

## 5. Stage 3b — Tm coexistence inputs
...
## 6. Stage 3c — Patch Tm into constants
...
```
The bracketing covers ±20% of Tm_rom. If the real Tm is outside this range,
widen `_BRACKET_FACTORS` in `pipeline/lammps_tm.py`.
...
**Check:**
```bash
# Verify r_sro is composition-specific (not a fixed 3.0)
...
## 8. Stage 4 — Submit diffusion jobs
...
| `compositions.csv` fractions ≠ 1 | Bug in Dirichlet sampling | Should not happen; open an issue |
